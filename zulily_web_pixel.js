...
// The referring event id is the last dl_user_data event id before hitting checkout.
// A list of the most recent event ids is stored in local storage
// Grab this list, and return the most recent value pushed onto it
// No user data event fires in checkout, so the last event id pushed is the last pre checkout dl user data event id
async function getReferringEventId() {
  try {
    const eventIds = await JSON.parse(await browser.localStorage.getItem('__zulily_pv_event_id_array'));
...
function getFormattedItems(event) {
...
analytics.subscribe('checkout_started', async (event) => {
  ...
  const reid =  await getReferringEventId();
  window.dataLayer.push({
   event: 'dl_begin_checkout',
   referring_event_id: reid,
   event_id: event.id,
   items: getFormattedItems(event),
